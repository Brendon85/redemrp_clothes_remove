...
local function drawTitle()
	if menus[currentMenu] then
		...
		if HasStreamedTextureDictLoaded("menu_textures") and HasStreamedTextureDictLoaded("generic_textures") then
			...
			DrawSprite("menu_textures", "translate_bg_1a", x, y+ 0.015, menus[currentMenu].width+ 0.045, titleHeight+ 0.05, 180.8, 000, 2, 2, 255, 1)
	...
	if multiplier then
		...
		if menus[currentMenu].currentOption == optionCount then
			if HasStreamedTextureDictLoaded("generic_textures") then
				...
				DrawSprite("generic_textures", "selection_box_bg_1d", x, y- 0.002, menus[currentMenu].width+ 0.03, buttonHeight, 0.0, 255, 255, 255, 255, 0)
			end
		else
			if HasStreamedTextureDictLoaded("generic_textures") then
				SetScriptGfxDrawOrder(1)
				DrawSprite("generic_textures", "selection_box_bg_1d", x, y- 0.002, menus[currentMenu].width+ 0.03, buttonHeight, 0.0, 0, 0, 0, 255, 0)
			end
		end
...
function WarMenu.CheckBox(text, checked, callback)
	if WarMenu.Button(text, checked and '☒' or '☐') then
		checked = not checked
